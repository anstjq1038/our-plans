...
export function DayMap({ events }: { events: PlanEvent[] }) {
  ...
  const focus = (i: number) => {
    ...
    if (!m) return;
    m.flyTo(L.latLng(spots[i].geo[0], spots[i].geo[1]), Math.max(m.getZoom(), 13), { duration: 0.6 });
  ...
  return (
    ...
    <div className="mb-4" onPointerDownCapture={(e) => e.stopPropagation()} onTouchStartCapture={(e) => e.stopPropagation()}>
      ...
      <p className="mt-1 text-[0.7rem] text-muted">핀 위치는 대략적이에요. 정확한 위치는 핀·일정의 지도 보기로 확인하세요.</p>
    </div>
  );
}
